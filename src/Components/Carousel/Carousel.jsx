  ...
  useEffect(() => {
    ...
    const getContainerWidth = () => {
      if (containerRef.current) {
        const tempContainerWidth = containerRef.current.getBoundingClientRect().width;
  ...
  return (
    <div className='carousel' style={{backgroundColor}}>
      ...
      <div  className='carousel-button-container'>
        {pageNo!==1 && <Button classNames={"carousel-button prev"} buttonName="&#8656;"
      onClick={()=> prevButton() } />}
      </div>
        <div className='carousel-slider' ref={containerRef} >

          {cardData.filter( (card, index) => index > pageData.prev-1 && index < pageData.next ).map( (card , index, arr) => {

            // console.log( "MP ", pageData, "Index ", index,arr)
              if( index === 0  ) {
                return <Card  heading={card.heading} content={card.body} img={card.img} key={card.id}  width={width}  hoverClassName={FIRST_ELEMENT}/>
              }else if( index === size -1 && arr.length >= size ){
                // console.log( "MP right ", pageData, "Index ", index)
                return <Card  heading={card.heading} content={card.body} img={card.img} key={card.id}  width={width}  hoverClassName={LAST_ELEMENT}/>
              }
            
            return <Card  heading={card.heading} content={card.body} img={card.img} key={card.id}  width={width}  hoverClassName={MID_ELEMENT}/>
        ...
        <div  className='carousel-button-container'>
        {lastPage!== pageNo && <Button classNames={"carousel-button next"}buttonName="&#8658;" onClick={()=> nextButton() }/>}
        </div>
      </div>
    </div>
  )
